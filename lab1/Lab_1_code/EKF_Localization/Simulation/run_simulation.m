...
function run_simulation(app, root, simulationfile)
    ...
    for timestep = 1:n_timesteps
        ...
        app.thetaField.Value = round(mu(3) / (pi) * 180);
        app.Error_x_Field.Value = pose_error(1);
        app.Error_y_Field.Value = pose_error(2);
        app.Error_theta_Field.Value = round(pose_error(3) / (pi) * 180);
    ...
    if stop_execution == 0
        ...
        met = mean(pose_errors(3,:) / (pi) * 180);
        maex = mean(abs(pose_errors(1,:)));
        maey = mean(abs(pose_errors(2,:)));
        maet = mean(abs(pose_errors(3,:) / (pi) * 180));
        ...
        subplot(3,1,3);
        plot(timesteps, pose_errors(3,:) / (pi) * 180);
